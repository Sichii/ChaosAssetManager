            ...
            var importedCount = await Task.Run(() =>
            {
                ...
                if (hasDyeablePalette)
                {
                    ...
                    for (var i = 0; i < epfs.Count; i++)
                        epfs[i] = epfs[i]
                            .ArrangeColorsForDyeableType(colorTable[0]);
                }
                ...
                var existingEntries = legend.GetEntries("item", ".epf")
                                            .ToDictionary(entry => entry.TryGetNumericIdentifier(out var id, 3) ? id : -1, entry => entry);
                ...
                var maxExistingPageId = existingEntries.Keys
                                                       .Where(k => k > 0)
                                                       .DefaultIfEmpty(0)
                                                       .Max();
