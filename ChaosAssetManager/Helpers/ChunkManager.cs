...
public sealed class MapChunk : IDisposable
{
    ...
    public MapChunk(int chunkX, int chunkY, int tileStartX, int tileStartY, int tileEndX, int tileEndY, int mapWidth, int mapHeight)
    {
        ChunkX = chunkX;
        ChunkY = chunkY;
        ...
        PixelBounds = ComputePixelBounds(tileStartX, tileStartY, tileEndX, tileEndY, mapHeight);
    ...
    /// </summary>
    private static SKRectI ComputePixelBounds(int startX, int startY, int endX, int endY, int mapHeight)
    {
        ...
        return new SKRectI(minPixelX, minPixelY, maxPixelX, maxPixelY);
    }
...
public sealed class ChunkManager : IDisposable
{
    ...
    public ChunkManager(int mapWidth, int mapHeight)
    {
        ...
        for (var cy = 0; cy < ChunksHigh; cy++)
        {
            for (var cx = 0; cx < ChunksWide; cx++)
            {
                ...
                Chunks[cx, cy] = new MapChunk(cx, cy, tileStartX, tileStartY, tileEndX, tileEndY, mapWidth, mapHeight);
            }
        }
    }
    ...
    /// </summary>
    public void MarkRangeDirty(int startX, int startY, int width, int height, LayerFlags layers)
    {
